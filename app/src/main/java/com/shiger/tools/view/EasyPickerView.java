...
public class EasyPickerView extends View {
    ...
    public EasyPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.EasyPickerView, defStyleAttr, 0);
        textSize = typedArray.getDimensionPixelSize(R.styleable.EasyPickerView_epvTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
        textColor = typedArray.getColor(R.styleable.EasyPickerView_epvTextColor, Color.BLACK);
        textPadding = typedArray.getDimensionPixelSize(R.styleable.EasyPickerView_epvTextPadding, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 20, getResources().getDisplayMetrics()));
        textMaxScale = typedArray.getFloat(R.styleable.EasyPickerView_epvTextMaxScale, 2.0f);
        textMinAlpha = typedArray.getFloat(R.styleable.EasyPickerView_epvTextMinAlpha, 0.4f);
        isRecycleMode = typedArray.getBoolean(R.styleable.EasyPickerView_epvRecycleMode, true);
        maxShowNum = typedArray.getInteger(R.styleable.EasyPickerView_epvMaxShowNum, 3);
        typedArray.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null != dataList && dataList.size() > 0) {
            ...
            canvas.drawLine(0, contentHeight / 3, contentWidth, contentHeight / 3, paint);
            canvas.drawLine(0, contentHeight / 3 * 2, contentWidth, contentHeight / 3 * 2, paint);
            ...
            for (int i = -half; i <= half; i++) {
                ...
                //change the color of curIndex
                if (index == curIndex) {
                    textPaint.setColor(textColor);
                }else {
                    textPaint.setColor(getResources().getColor(R.color.line_gray));
                }
